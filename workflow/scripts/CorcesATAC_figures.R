...
CorcesATAC_enrichment_results_azimuth_path <- snakemake@input[["enrichment_results_azimuth"]]
CorcesATAC_enrichment_results_reactome_path <- snakemake@input[["enrichment_results_reactome"]]
...
atac_enrichment_azimuth_path <- snakemake@output[["enrichment_azimuth_plot"]]
atac_enrichment_reactome_path <- snakemake@output[["enrichment_reactome_plot"]]
...
# Create UMAP plots
atac_umap_plot <- umap_plot(CorcesATAC_umap_coords_path, atac_umap_path, title = "Chromatin accessibility")
...
atac_df_formatted <- create_atac_enrichment_df(CorcesATAC_enrichment_results_azimuth_path, fdr_threshold)
atac_heatmap_df <- prepare_for_heatmap(df_formatted = atac_df_formatted,
                                      fdr_threshold = fdr_threshold,
                                      tissues_to_keep = c("PBMC", "Bone Marrow"))
atac_enrichment_azimuth_plot <- plot_enrichment_heatmap(
    heatmap_df = atac_heatmap_df, 
    fig_path = atac_enrichment_azimuth_path,
    fill_lab = "log2(fold change)",
    size_lab = "-log10(p-adj.)",
    title = "Chromatin accessibility",
    ylabel = "Enrichment term\n(GREAT, Azimuth 2023)"
)

atac_df_formatted <- create_atac_enrichment_df(CorcesATAC_enrichment_results_reactome_path, fdr_threshold)
atac_heatmap_df <- prepare_for_heatmap(df_formatted = atac_df_formatted,
                                      fdr_threshold = fdr_threshold,
                                      tissues_to_keep = NULL)
atac_enrichment_reactome_plot <- plot_enrichment_heatmap(
    heatmap_df = atac_heatmap_df, 
    fig_path = atac_enrichment_reactome_path,
    fill_lab = "log2(fold change)",
    size_lab = "-log10(p-adj.)",
    title = "Chromatin accessibility",
    ylabel = "Enrichment term\n(GREAT, Reactome)"
...
atac_crosspred_p <- plot_crossprediction_from_adjacency(
    ...
    modality_label = "Chromatin accessibility",
)